...
from util.validator import validator_form_login


class ColumnFormLogin(ft.Column):
    ...
    async def submit_form(self, e):
        try:
            self.user = await validator_form_login(self, e)
            if self.user:
                self.callback_mudar_tab(0)
                self.page.open(SuccessSnackBar('Login realizado com sucesso!'))
